...
if(instance_exists(objPlayerClone)) {
    if(alarm[1] < 0) {
        ...
        objPlayerClone.shootDirection = "right";
    }
}

// Si tiene disparo estatico lo maneja en vez de disparar
if(Bonus_Static_Shoot == true) {
    with(obj_Jugador_Disparo_Estatico) Velocidad_H += Aceleracion;
...
if(global.characterSelected == 10) {
    if(instance_exists(objFleshSoul)) {
        with(objFleshSoul)  Velocidad_H += Aceleracion;
        bulletShootAnimation = false;
        exit;
    }
}
...
if(player_item_331) and (bullet_hell_bullets_total <= 0) {
	bullet_hell = 1;
	if(bullet_hell_bullets_total <= 0) bullet_hell_direction = 1;
...
if(player_item_325) or (player_has_pet(87) and chance(5)) {
	...
	var nova = instance_create(x+10,y+2+3*Disparo_Move,objBulletOrb);
	nova.dir = 0;
	exit;
}
...
if(player_item_122) {
    if((alarm[9]<0)) {
        Laser_Direction = 0;
        alarm[9] = 60;
        var lazer = bullet_spawn(x,y,0,0,0);
		lazer.Parent = id;
    }
    exit;
}
...
var Disparo = instance_create( x+10, y+2 + 3*Disparo_Move + (-Bonus_100Punch/2 + random(Bonus_100Punch)), Tipo_Municion );
Disparo.H_SPEED = + Velocidad_Disparo + Angulo_H;
Disparo.V_SPEED = Angulo_V;
Disparo.Direccion_Relativa = 1;
if(player_item_298) Disparo.r298 = true;

// Increase rage
if(player_item_321) rage += 1;

// Crear efecto de disparo
var FX = create_effect(x+14,y-1 + 3*Disparo_Move,sprSplatMini,0.4);

...
if(instance_exists(objPlayerClone)) {
    if(alarm[1] < 0) {
        ...
        objPlayerClone.shootDirection = "down";
    }
}
...
if(player_item_331) and (bullet_hell_bullets_total <= 0) {
	bullet_hell = 1;
	if(bullet_hell_bullets_total <= 0) bullet_hell_direction = 4;
...
if(player_item_122) {
    if((alarm[9]<0)) {
        Laser_Direction = 270;
        alarm[9] = 60;
        var lazer = bullet_spawn(x,y,0,0,270);
		lazer.Parent = id;
    }
    exit;
}

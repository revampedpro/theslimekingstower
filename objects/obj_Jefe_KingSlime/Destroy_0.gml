...
if(instance_exists(obj_Jugador)) and (global.Modo_Juego == "Normal") {
    if(global.Mapa_Tipo == 6) {
        ...
        ini_open("save.ini");
        ini_write_real("SC"+string(global.characterSelected), "1", true);
        ini_close();
        global.CharacterUnlocked[global.characterSelected,1] = true;
        
        // Unlock Jade
        if(!check_character_unlocked(1,0)) unlock_character(1);
		
		// Unlock Slimer
		if(!check_character_unlocked(11,0)) {
			with(obj_Jugador) {
				if(player_has_shot == false) unlock_character(11);
			}
		}
        
        // [STEAM] Basement Achievement
        steam_unlock("L_BASEMENT");
        
        // Complete challenges that doesn't require basement
        if(global.Modo_Challenge_Activo == true) {
            if(get_challenge(global.Modo_Challenge,"basement") == false) {
                complete_challenge(global.Modo_Challenge);
                instance_create(32+room_width/2,-24+room_height/2,objSlimekingCrown);
            }
        } else {
            instance_create(32+room_width/2,-24+room_height/2,objSlimekingCrown);
        }
    ...
    if(global.Mapa_Tipo == 11) {
		flash(30,c_aqua);
        instance_create(32+room_width/2,-24+room_height/2,objSlimekingMoustache);
        repeat(2) { 
            with(parent_Enemigo) { if(HP != 0) { HP = 0; instance_destroy(); } }
            with(parent_Jefe)    { if(HP != 0) { HP = 0; instance_destroy(); } }
        }
        ini_open("save.ini");
        ini_write_real("SC"+string(global.characterSelected), "2", true);
        global.CharacterUnlocked[global.characterSelected,2] = true;
        ini_close();
        
        // Check if you unlockd The Collector
        if(global.characterSelected == 2) and (!check_character_unlocked(4,0)) unlock_character(4);
        
        // Complete challenges that require basement
        if(global.Modo_Challenge_Activo == true) {
            complete_challenge(global.Modo_Challenge);
        }
    ...
    //GoogleAnalytics_SendEvent("ingame","slimeking kill");
    
}

if(instance_exists(obj_Jugador)) and (global.Modo_Juego == "Endless") {

    // Llevar al jugador a basement
    if(global.Mapa_Tipo == 6) {
        /*
    ...
    // Llevar al jugador al portal
    if(global.Mapa_Tipo == 11) {
        instance_create(room_width/2,-24+room_height/2,objSlimekingMoustache);
        repeat(2) { 
            with(parent_Enemigo) { if(HP != 0) { HP = 0; instance_destroy(); } }
            with(parent_Jefe)    { if(HP != 0) { HP = 0; instance_destroy(); } }
        }
        /*
        global.Mapa_Direccion = -1; // Goin down
        global.Mapa_Tipo = 12; // Go to <Portal>
        global.Mapa_Nivel = 0;
        dungeon_next_level();
        */
    }

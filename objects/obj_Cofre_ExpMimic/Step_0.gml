...
if(sprite_index == spr_Cofre_ExpMimic_Reposo) {
...
if(sprite_index == spr_Cofre_ExpMimic_Esperando) {
    if(distance_to_object(obj_Jugador)>=30) {
        sprite_index = spr_Cofre_ExpMimic_Reposo;
        image_speed = 0.2;
        image_index = 0;
    }
}
...
// Coger el item
if(distance_to_object(obj_Jugador) < 2 and obj_Jugador.alarm[4]<0 and global.Jugador_Monedas > 0 and alarm[1] < 0 and sprite_index != spr_Cofre_ExpMimic_Comiendo) {

    // Porcentaje de exito (50%)
    global.Jugador_Monedas--;
    audio_play_sound(snd_Moneda, 0, 0);
    audio_play_sound(sndMimicEating,0 ,0);
    alarm[1] = 25;
    sprite_index = spr_Cofre_ExpMimic_Comiendo;
    image_index = 0;

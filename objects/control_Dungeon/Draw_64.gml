...
if(global.Quality < 2) {
	...
	for(var i = 0; i < s; i++) {
		var m = global.Dungeon_Map[| i];
		draw_sprite_ext(m[? "spr"],m[? "index"],m[? "x"],m[? "y"],m[? "s"],m[? "s"],0,-1,m[? "alpha"]);
	}
	
	/*
	var s = array_length_1d(map_spr);
	for(var i = 0; i < s; i++) {
		draw_sprite_ext(map_spr[i],map_index[i],map_x[i],map_y[i],map_s[i],map_s[i],0,-1,map_alpha[i]);
	}
	*/
	...
	for(var i = 0; i < l; i++) {
		...
		if(m[? "t"] == 1) draw_sprite_ext(spr_UI_Map_Icons, 29,  m[? "x"], m[? "y"], m[? "s"], m[? "s"], 0, c_white, m[? "a"]);
		if(m[? "t"] == 2) draw_sprite_ext(spr_UI_Map_Icons, 29,  m[? "x"], m[? "y"], m[? "s"], m[? "s"], 0, c_white, m[? "a"]);
		ds_map_destroy(m);
	}
	...
	for(var i = 0; i < 5; i++) {
	    for(var j = 0; j < 5; j++) {
	        var dx = global.Posicion_i - 2 + i;
	        var dy = global.Posicion_j - 2 + j;
			if(dx = global.Posicion_i and dy = global.Posicion_j) and ((Dungeon[dx,dy] == 1) or (Dungeon[dx,dy] == 29) or (Dungeon[dx,dy] == 2)) draw_sprite_ext(spr_UI_Map_Icons, 20, 0 + mx+2+i*(sep) + 8*(1-re*wav), my+2+j*seph + 8*(1-re*wav), 2*re*wav, 2*re*wav, 0, c_white, 1);
		}
	}
